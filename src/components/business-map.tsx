...
const tiles: Record<TileKey, { url: string; attribution: string; label: string; sub: string }> = {
  "neshan-like": {
    label: "نقشه خیابانی (نشان-استایل)",
    sub: "خیابان‌ها، کوچه‌ها، پل‌ها",
    url: "https://{s}.basemaps.cartocdn.com/rastertiles/voyager/{z}/{x}/{y}{r}.png",
    attribution: "© OSM © CARTO • خیابان‌های کامل",
  },
  "google-like": {
    label: "گوگل استایل روشن",
    sub: "روشن و خوانا",
...
export default function BusinessMap({ businesses, onSelectBusiness, center, className, highlightedId, relatedIds = [], showConnections = true }: Props) {
  ...
  useEffect(() => {
    ...
    const L = leaflet;
    const map = L.map(containerRef.current, { zoomControl: false, attributionControl: false }).setView([currentCenter.lat, currentCenter.lng], 14);
    L.control.zoom({ position: "bottomleft" }).addTo(map);
    L.control.attribution({ position: "bottomright", prefix: "" }).addTo(map);
    tileLayerRef.current = L.tileLayer(tiles[activeTile].url, { attribution: tiles[activeTile].attribution, maxZoom: 19 }).addTo(map);
    mapRef.current = map;
    return () => {
      map.remove();
      mapRef.current = null;
  ...
  useEffect(() => {
    ...
    const iconFor = (record: BusinessRecord) => {
      ...
      return L.divIcon({
        className: "",
        html: `<div style="background:${color}; width:${size}px; height:${size}px; border-radius:12px; display:grid; place-items:center; color:white; font-weight:900; font-size:${size > 36 ? 15 : 13}px; border:3px solid white; ${ring} ${pulse}">${record.name
          .slice(0, 1)
          .toUpperCase()}</div>${
          badge ? `<div style="position:absolute; top:${size + 2}px; left:50%; transform:translateX(-50%); background:${isHighlighted ? "#0f172a" : isRelated ? "#7c3aed" : "#111"}; color:white; font-size:9px; font-weight:800; padding:2px 7px; border-radius:999px; white-space:nowrap; box-shadow:0 4px 10px rgba(0,0,0,.25);">${badge}</div>` : ""
        }`,
    ...
    businesses
      ...
      .forEach((b) => {
        ...
        bounds.push([lat, lng]);
        const marker = L.marker([lat, lng], { icon: iconFor(b), zIndexOffset: highlightedId === b.id ? 1000 : relatedIds.includes(b.id) ? 800 : 0 }).addTo(map);
        ...
        marker.bindPopup(
          `<div dir="rtl" style="font-family:Tahoma; min-width:240px; text-align:right">
    ...
    else if (bounds.length > 1) map.fitBounds(bounds as any, { padding: [28, 28], maxZoom: 16 });
    else if (bounds.length === 1) map.setView(bounds[0] as any, 16);

    if (typeof document !== "undefined" && !document.getElementById("mapPulseStyle")) {
      const style = document.createElement("style");
      style.id = "mapPulseStyle";
      style.innerHTML = "@keyframes mapPulse {0%{transform:scale(1)}50%{transform:scale(1.12)}100%{transform:scale(1)}}";
  ...
  return (
    <div className={className ?? "relative w-full"}>
      <div className="flex flex-wrap items-center gap-2 pb-3">
        ...
        <button onClick={() => setShowDual(!showDual)} className="ml-auto rounded-xl bg-[#0f172a] px-3 py-2 text-[10px] font-bold text-white hover:bg-black">
          {showDual ? "پنهان‌سازی Google/Neshan" : "نمایش همزمان Google + نشان (خیابان‌ها)"}
        </button>
      </div>
      ...
      <div className="relative h-[520px] w-full overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-inner">
        ...
        {!isLoaded && <div className="absolute inset-0 grid place-items-center bg-slate-50 text-[11px] font-bold text-slate-500 z-20">در حال بارگذاری نقشه ارتباطی...</div>}
        <div className="pointer-events-none absolute bottom-3 right-3 z-20 flex flex-wrap gap-1">
          ...
          <span className="rounded-lg bg-violet-600/90 px-2 py-1 text-[9px] font-bold text-white shadow">🟣 مرتبط هم‌خیابان - خط اتصال</span>
      ...
      {showDual && (
        <div className="mt-3 grid gap-3 lg:grid-cols-2">
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <div className="flex items-center justify-between bg-slate-50 px-3 py-2"><span className="text-[11px] font-extrabold text-slate-700">Google Maps - خیابان‌ها و مسیریابی بین مشاغل</span><a href={`https://www.google.com/maps/search/?api=1&query=${currentCenter.lat},${currentCenter.lng}`} target="_blank" className="text-[10px] font-bold text-indigo-600 hover:underline">کامل ↗</a></div>
            <div className="relative h-[260px] bg-slate-100"><iframe title="Google Maps" src={googleEmbed} className="h-full w-full border-0" loading="lazy" /></div>
          </div>
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white">
            <div className="flex items-center justify-between bg-[#e6f5f2] px-3 py-2"><span className="text-[11px] font-extrabold text-[#0f766e]">نشان - خیابان‌های دقیق + nshn.ir قابل اتصال</span><a href={`https://nshn.ir/${currentCenter.lat},${currentCenter.lng}`} target="_blank" className="text-[10px] font-bold text-teal-700 hover:underline">nshn.ir ↗</a></div>
            <div className="relative h-[260px] bg-[#edf4f3]"><iframe title="Neshan Maps" src={neshanEmbed} className="h-full w-full border-0" loading="lazy" /><div className="pointer-events-none absolute bottom-2 right-2 rounded bg-white/95 px-2 py-1 text-[9px] font-bold text-slate-700 shadow">خط‌چین‌ها = اتصال خدماتی بین کسب‌وکارهای همسایه</div></div>
          </div>
        </div>
      )}
    </div>
